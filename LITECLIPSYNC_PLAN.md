...
- **双模式同步引擎**：LiteClipSync 实时推送 + WebDAV 标准轮询
- **单文件部署**，零运行时依赖
...
│                                                      │
│  ┌────────────── SYNC ENGINE ────────────────────┐   │
│  │ Mode 1: LiteClipSync (WS实时推送 + REST文件传输)   │   │
...
**WebSocket 消息（LiteClipSync 模式专用）：**

```json
// 客户端→服务端
{ "type": "Auth", "token": "base64(username:password)" }

{ "type": "LiteClipSync", "payload": { ClipPayload } }
...
| 功能 | 说明 | 计划 |
|------|------|------|
| LiteClipSync 服务端 | 自建实时推送服务端 | Phase 4 |
| LiteClipSync WS 模式 | 客户端连自建服务端 | Phase 5 |
...
|------|------|
| WS 连接 + 鉴权 | 发送 Auth 消息 |
| LiteClipSyncEngine | 实现 SyncEngine trait |
| WS 发送剪贴板 | 发送 ClientMsg::LiteClipSync |
| WS 接收广播 | 过滤 source_device_id |
| 文件走 REST | 大文件走 HTTP GET/PUT |
| 断连重连 | 指数退避 |
| 模式切换 | config `mode: "liteclipsync" \| "webdav"` |
...
| 维度 | SyncClipboard | LiteClipSync | 优势方 |
|------|-------------|----------|--------|
| 运行时依赖 | .NET 8 Runtime (~80MB) | 单文件，零依赖 | LiteClipSync |
| 服务端镜像 | ~100MB (ASP.NET) | ~15MB (Axum) | LiteClipSync |
...
| 部署复杂度 | 高 | 低 | LiteClipSync |
| 代码可控性 | 第三方维护 | 完全掌控 | LiteClipSync |
