...
async fn apply_remote_clipboard(
    ...
    let (content, real_hash) = match payload.content_type.as_str() {
        ...
        "Image" => {
            ...
            match client::http_download_to_file(config, &payload.data_name, &std::env::temp_dir().join(format!("liteclipsync_dl_{}", payload.data_name))).await {
                Ok(()) => {
                    let data = std::fs::read(std::env::temp_dir().join(format!("liteclipsync_dl_{}", payload.data_name))).unwrap_or_default();
        ...
        "File" => {
            ...
            let dest = std::env::temp_dir().join(format!("liteclipsync_dl_{}", payload.data_name));
