...
#[derive(Parser)]
#[command(name = "liteclipsync", about = "LiteClipSync Windows client")]
struct Cli {
...
fn main() {
    ...
    tracing::info!(
        "LiteClipSync starting: server={}, device={} ({})",
    ...
    let tray = match TrayIconBuilder::new()
        .with_tooltip(if cfg.language == "zh" { "LiteClipSync - 连接中..." } else { "LiteClipSync - connecting..." })
    ...
    unsafe {
        ...
        let title: Vec<u16> = OsStr::new("LiteClipSync Hidden\0").encode_wide().collect();
        CreateWindowExW(
            0,
            class.as_ptr(),
            title.as_ptr(),
...
fn update_tray(
    ...
    let tooltip = format!("LiteClipSync -{ts}");
