...
impl Database {
    pub fn open(path: &Path) -> anyhow::Result<Self> {
        ...
        let conn = Connection::open(path.join("liteclipsync.db"))?;
    ...
    fn migrate(&self) -> anyhow::Result<()> {
        ...
        // Alter table statements may fail safely if columns already exist
        for alter in [
            "ALTER TABLE clipboard_history ADD COLUMN has_data INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE clipboard_history ADD COLUMN text TEXT NOT NULL DEFAULT ''",
        ] {
            if let Err(e) = conn.execute_batch(alter) {
                warn!("DB migration alter skipped (likely exists): {e}");
            }
        }
        Ok(())
    }
    ...
    pub fn register_device(&self, device_id: &str, name: &str) {
        ...
        if let Err(e) = conn.execute(
            ...
            error!("DB register_device failed: {e}");
        }
    }
    ...
    pub fn save_profile(&self, profile: &ProfileDto, device_id: &str) -> bool {
        let conn = lock_db(&self.conn);
        match conn.execute(
            ...
            Ok(_) => true,
            Err(e) => { error!("DB save_profile failed: {e}"); false }
        }
    }
    ...
    pub fn get_backlog(&self, device_id: &str) -> Vec<ProfileDto> {
        ...
        let mut stmt = match conn.prepare(
            ...
            Ok(s) => s,
            Err(e) => { error!("DB get_backlog prepare failed: {e}"); return vec![]; }
        };
        let rows = match stmt.query_map(rusqlite::params![last_seen, device_id], |row| {
            Ok(ProfileDto {
                hash: row.get(0)?,
                content_type: row.get(1)?,
                text: row.get::<_, String>(2).unwrap_or_default(),
            ...
            Ok(r) => r,
            Err(e) => { error!("DB get_backlog query_map failed: {e}"); return vec![]; }
        };
        rows.filter_map(|r| r.ok()).collect()
    }
}

fn lock_or_err<'a, T>(mtx: &'a Mutex<T>) -> Result<std::sync::MutexGuard<'a, T>, &'static str> {
    mtx.lock().map_err(|_| "mutex poisoned")
}

fn lock_db<'a, T>(mtx: &'a Mutex<T>) -> std::sync::MutexGuard<'a, T> {
    match mtx.lock() {
        Ok(g) => g,
        Err(poisoned) => {
            error!("DB mutex poisoned, recovering");
            poisoned.into_inner()
        }
    }
}
