...
class SyncManager(private val ctx: Context) {
    ...
    val cfg = (ctx.applicationContext as LiteClipSyncApp).config
        ...
        while (cycling) {
            ...
            while (cycling && w.connected) {
                try {
                    ...
                    Log.e("LiteClipSync", "sync crash: ${e.message}", e)
                }
            }
            Log.e("LiteClipSync", "sync: disconnected, reconnecting...")
    ...
    private suspend fun sendClip(w: WsClient, h: String, txt: String, sz: Long) {
        ...
        if (sz <= cfg.autoSyncMaxBytes) {
            w.send(buildLiteClipSync(ProfileDto("Text", h, text = txt, size = sz), cfg.deviceId))
        } else {
            val name = "${safePrefix(txt)}_${h.take(8)}.txt"
            http.upload(name, txt.toByteArray())
            w.send(buildLiteClipSync(ProfileDto("Text", h, hasData = true, dataName = name, size = sz), cfg.deviceId))
        }
    ...
    suspend fun uploadText(text: String): String {
        ...
        if (text.toByteArray().size <= cfg.autoSyncMaxBytes) {
            wl.send(buildLiteClipSync(ProfileDto("Text", h, text = text, size = text.length.toLong()), cfg.deviceId))
            ...
            wl.send(buildLiteClipSync(ProfileDto("Text", h, hasData = true, dataName = name, size = text.length.toLong()), cfg.deviceId))
    ...
    suspend fun fetchLatest(): ProfileDto? {
        return try {
            ...
            if (!resp.isSuccessful) { Log.w("LiteClipSync", "fetch HTTP ${resp.code}"); return null }
        ...
        } catch (e: Exception) { Log.w("LiteClipSync", "fetch error: ${e.message}"); null }
    }
    ...
    suspend fun uploadFromUri(uri: android.net.Uri, cr: android.content.ContentResolver,
        ...
        wl.send(buildLiteClipSync(ProfileDto("File", h, hasData = true, dataName = name, size = totalSize), cfg.deviceId))
    ...
    private suspend fun checkClipboard(w: WsClient?) {
        ...
        if (clipText != null && sha256(clipText.toByteArray()) == lastHash && ShizukuShell.available()) {
            ...
            if (fresh != null && fresh.isNotEmpty() && fresh != clipText) {
                Log.e("LiteClipSync", "stale cm=[${clipText}] → shizuku=[${fresh}]")
                clipText = fresh
            }
        }
        ...
        when (p.contentType) {
            "Text" -> {
                ...
                if (txt.length > MAX_CLIP_BYTES) {
                    Log.w("LiteClipSync", "applyRemote: text too large (${txt.length} chars), skip")
                    return
                }
                val h = sha256(txt.toByteArray())
                if (h == lastHash) return  // already synced, skip to avoid overwriting local edits
                lastHash = h
                cm.setPrimaryClip(ClipData.newPlainText("LiteClipSync", txt))
            }
        }
    }
